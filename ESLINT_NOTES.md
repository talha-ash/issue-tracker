...
### Lines 20–23: Shared Presets (applied to ALL files)
...
> **Note**: `nodePlugin` is **not** a global preset — it's scoped to `packages/backend/**` only (see Lines 102–109).
...
### Lines 94–100: Package Overrides
...
### Lines 101–112: Node.js Plugin (Backend Only)
...
Scoped to `packages/backend/**` only — Node.js rules are irrelevant for browser apps (Next.js, TanStack Start) and the shared UI package. The `flat/recommended-module` preset is specifically for ESM projects (not CommonJS). It catches deprecated Node APIs (`fs.exists`), unsupported syntax for your Node version, and ensures `engines` field compatibility.

- **`n/no-missing-import`: off** — This rule checks if imported modules can be resolved by Node. But in this project, bundlers handle resolution — they understand path aliases, workspace packages, and framework-specific imports that Node can't resolve on its own.
...
- **`n/no-extraneous-import`: allowModules `['neverthrow']`** — `neverthrow` lives in the root `package.json` and is hoisted by pnpm, but backend's own `package.json` doesn't list it. The rule walks up to the nearest `package.json` and checks its `dependencies` field. Using `allowModules` (not `resolvePaths`) because `resolvePaths` only helps locate modules on disk — it can't make the rule see root-level deps for a nested package.

---

### Line 113: Prettier (MUST be last)
